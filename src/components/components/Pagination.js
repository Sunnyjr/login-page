...
function Home() {
    ...
    const pageCount = listData ? Math.ceil(listData.length / pageSize) : 0; //s3// it multiple the length by pagesize 42/6 =7 // Math. ceil() function always rounds a number up to the next largest integer. 
    if (pageCount === 1) return null;  //s4
    const pages = _.range(1, pageCount + 1) //s6 //_.range return array taht include started the page from 1 and continue to end page i.e 7 

    const pagination = (pageNo) => { //s13
        setCurrentPage(pageNo);
        const startIndex = (pageNo - 1) * pageSize;  //its dived the 6 items per page
            ...
            <div className="row p-2" >
                {list === true || grid === false ?
                    paginatedPosts.map((item, key) => { //s10 change listData to paginatedpost
                        ...
                        return (
                            <div class="column" style={{ width: "15rem", border:"2px solid" ,marginBottom:"8px", borderRadius:'6px'}} key={key}>
                                {item.image}
                                <div class="card-body">
                        ...
                        { //s7 
                            pages.map((page) => (
                                <li class={ page === currentPage ? "page-item active" : "page-item" }>
                                 <a class="page-link" onClick={() => pagination(page)} href="#">{page}</a>
                                </li> //s12
                            ))
                        }                          
                        {                                                                               
